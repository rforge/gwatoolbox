...
By default the \emph{gwasformat()} doesn't calculate the inflation factor and doesn't apply the genomic control.
This behaviour can be modified for every input file in the input script using the command \textbf{GC}\index{GC}/\textbf{GENOMICCONTROL}\index{GENOMICCONTROL} as specified below:
\begin{table}[!ht]
\begin{center}
\begin{tabular}{|l|p{8cm}|}
\hline
...
ON & The inflation factor is calculated. Values in \emph{PVALUE} and \emph{STDERR} columns are corrected and saved to the new columns \emph{PVALUE\_gc} and \emph{STDERR\_gc}, accordingly. Has no effect if \emph{PVALUE} column is not present. \\
numeric value & The inflation factor is assumed to be equal to the specified \emph{numeric value}. Values in \emph{PVALUE} and \emph{STDERR} columns are corrected and saved to the new columns \emph{PVALUE\_gc} and \emph{STDERR\_gc}, accordingly. \\
\hline
\end{tabular}
...
By default, the \emph{gwasformat()} computes the effective sample size based on \emph{IMP\_QUALITY} and \emph{N} columns.
The computed values are saved to the new column \emph{N\_effective}.
