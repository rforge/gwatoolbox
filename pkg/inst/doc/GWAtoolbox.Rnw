...
The GWAS data files from distinct studies usually have different formatting.
This includes differences in naming and ordering of columns, column separators.
For the automated analysis of data it is more convenient to have uniform format across all the files of interest.
The \emph{gwasformat()} function and its prallel version \emph{pgwasformat()} allow to quickly transform GWAS data files into a uniform format.
In addition to the possiblity to rename and re-order columns, these functions calculate effective sample size, inflation factors and apply genomic control of P-values and standard errors.
The column separator is set to tabulation in all processed files.

\subsubsection{Input Script}
The structure of input script for \emph{gwasformat()} is identical to the structure of input script for \emph{gwasqc()}.
Additional commands are supported allowing to rename and reorder columns, enable/disable genomic control.
...
# Description (meaning) of columns
...
ORDER	ON	chrom bp rsId strand_genome coded_all noncoded_all AF_coded_all 
...
The new names for columns in the GWAS data file are specified in the input script with the command \textbf{RENAME}\index{RENAME}.
